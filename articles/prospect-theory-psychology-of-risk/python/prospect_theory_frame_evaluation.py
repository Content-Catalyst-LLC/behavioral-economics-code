...
pd.DataFrame(rows).to_csv(REG / "python_prospect_theory_frame_estimates.csv", index=False)

df["lambda_quartile"] = pd.qcut(df["lambda_loss"], 4, labels=["Q1", "Q2", "Q3", "Q4"])
df["gamma_quartile"] = pd.qcut(df["gamma_weight"], 4, labels=["Q1", "Q2", "Q3", "Q4"])
df["rho_quartile"] = pd.qcut(df["rho_crra"], 4, labels=["Q1", "Q2", "Q3", "Q4"])
...
for group_col, out_name in [
    ("lambda_quartile", "python_prospect_theory_lambda_heterogeneity.csv"),
    ("gamma_quartile", "python_prospect_theory_probability_weighting_heterogeneity.csv"),
    ("rho_quartile", "python_prospect_theory_crra_heterogeneity.csv"),
    ("wealth_quartile", "python_prospect_theory_wealth_heterogeneity.csv"),
    ("security_quartile", "python_prospect_theory_security_heterogeneity.csv"),
    ("trust_quartile", "python_prospect_theory_trust_heterogeneity.csv"),
]:
...
prior_loss.to_csv(REG / "python_prospect_theory_prior_loss_heterogeneity.csv", index=False)

print("Wrote Python prospect-theory frame-effect and heterogeneity summaries.")
